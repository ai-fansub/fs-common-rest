...
public class RestApiService<T> {
    ...
    private Object getResponseResultData(ResponseEntity<T> response) {
        if (null == response.getBody()) {
            throw new FsCommonException(ErrorType.ERROR, ServiceStatusCode.ERROR_NETWORK, "response is null!!");
        }
        ...
        if (100 != code && 200 != code) {
            throw new FsCommonException((String) resultData.get("message"));
        }
